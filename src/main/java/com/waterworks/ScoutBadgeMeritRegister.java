...
public class ScoutBadgeMeritRegister extends Activity{
	...
	public void init(){
		...
		isInternetPresent = Utility.isNetworkConnected(ScoutBadgeMeritRegister.this);
		if (!isInternetPresent) {
			onDetectNetworkState().show();
		} else {
			new Get_UpcomingGirlsPrograms().execute();
		}
	}
	...
	class Get_UpcomingGirlsPrograms extends AsyncTask<Void, Void, Void>{
		ProgressDialog pd;
		...
		protected void onPreExecute() {
			// TODO Auto-generated method stub
			super.onPreExecute();
			pd = new ProgressDialog(ScoutBadgeMeritRegister.this);
		...
		protected Void doInBackground(Void... params) {
			...
			String responseString = WebServicesCall
					.RunScript(AppConfiguration.DOMAIN+AppConfiguration.Get_UpcomingSwimmingMeritBadgesPrograms, param);
			try {
				JSONObject reader = new JSONObject(responseString);
				SwimCampData = reader.getString("Success");
				if(reader.toString().contains("Msg")){
					Title = reader.getString("Msg");
				}
		...
		protected void onPostExecute(Void result) {
			...
			if (!isInternetPresent) {
				...
				new Get_GirlsProgramsRegistrations().execute();
			}
			pd.dismiss();
		}
	}
	...
	protected void onResume() {
		...
		txt_register.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent i = new Intent(mContext,ScoutBadgeMeritRegister1.class);
	...
	class Get_GirlsProgramsRegistrations extends AsyncTask<Void, Void, Void>{
		ProgressDialog pd;
		...
		protected Void doInBackground(Void... params) {
			...
			String responseString = WebServicesCall
					.RunScript(AppConfiguration.DOMAIN+AppConfiguration.Get_SwimmingMeritBadgesRegistrations,param);
			...
			try {
				...
				if (SwimCampData.toString().equals("True")) {
					...
					for (int i = 0; i < jsonMainNode.length(); i++) {
						...
						Get_EndDate.add(jsonChildNode.getString("End Date"));
					}
				} else {
	...
	public void setLayout_Upcoming(){
		...
		for (int position = 0; position <_Name.size(); position++) {
			...
			end_date.setText(Html.fromHtml("<b>End Date: </b>"+_EndDate.get(position)));
	...
	public void SetLayout(){
		for (int position = 0; position <Get_Name.size(); position++) {
			...
			end_date.setText(Html.fromHtml("<b>End Date: </b>"+Get_EndDate.get(position)));
